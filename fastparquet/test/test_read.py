...
def test_converted_type_null():
    ...
    p = fastparquet.ParquetFile(
        os.path.join(TEST_DATA, "test-converted-type-null.parquet")
    )
    data = p.to_pandas()
...
def test_null_plain_dictionary():
    ...
    column."""
    p = fastparquet.ParquetFile(os.path.join(TEST_DATA, "test-null-dictionary.parquet"))
    data = p.to_pandas()
    expected = pd.DataFrame([{"foo": None}] + [{"foo": "bar"}, {"foo": "baz"}] * 3)
    for col in data:
...
def test_dir_partition():
    ...
    for cat, catnum in product(['fred', 'freda'], [1, 2, 3]):
        assert (df.num[(df.cat == cat) & (df.catnum == catnum)].tolist()) == out.num[
            (out.cat == cat) & (out.catnum == catnum)
        ].tolist()
...
def test_statistics(tempdir):
    ...
    df = pd.DataFrame(
        {'a': s, 'b': s.astype('category'), 'c': s.astype('category').cat.as_ordered()}
    )
...
def test_index(tempdir):
    ...
    df = pd.DataFrame({'a': s, 'b': s.astype('category'), 'c': range(60, 0, -1)})
    ...
    for column in df:
        ...
        pd.testing.assert_frame_equal(
            out, d2, check_categorical=False, check_index_type=False, check_dtype=False
        )
...
def test_rle_dict():
    ...
    pf = fastparquet.ParquetFile(
        os.path.join(TEST_DATA, 'repeated_no_annotation.parquet')
    )
    out = pf.to_pandas()
...
def test_map_multipage(tempdir):
    ...
    last_row_keys = [
        u'protests',
        u'gas mask',
        u'Pot & Painting Party',
        u'Denver',
        u'New Year',
        u'Anderson Cooper',
        u'gas mask bonk',
        u'digital media',
        u'marijuana leaf earrings',
        u'Screengrab',
        u'gas mask bongs',
        u'Randi Kaye',
        u'Lee Rogers',
...
def test_map_last_row_split(tempdir):
    ...
    first_split_row_keys = [
        ...
        u'Vladimir Putin',
        u'Hill two',
        u'The New York Times and Washington Post',
        u'tweet',
        u'guest bedroom',
        u'Susie Tompkins Buell',
        u'private law',
        u'Tammy Bruce',
        u'Obama Presidential Library',
        u'Fox News',
        u'President Trump',
        u'John Kerry',
        u'Vanity Fair',
        ...
        u'New Year',
        u'New York City',
        u'Media Research center',
        u'Neue Osnabruecker Zeitung daily newspaper',
        u'Berlin',
        u'gegen diese Taten vorgehen',
        u'safety',
        u'Jarrett Blanc',
        u'Tehran',
        u'America',
        ...
        u'pussy hats',
        u'wurden bislang leider vereinzelt sexuelle \xdcbergriffe gemeldet',
        u'Roger Cohen',
        u'u.s.',
        u'Donald Trump',
        u'Emily Shire',
        ...
        u'Associated Press',
        u'Iran',
        u'out-of-pocket expenses',
        u'Neue Osnabruecker Zeitung',
        u'lizards',
        u'Carlos Leon',
        u'Polizei Berlin Einsatz',
        u'Russia',
        u'Russian',
        u'Berlin Wall',
        u'Obama',
        ...
        u'American',
        u'Jeff Bell',
        u'Heat Street and National Review Online',
        u'Dan Merica',
        u'Tel Aviv',
        u'Wielding Money',
        u'anxiety',
        ...
        u'Paul Krugman',
        u'FoxNews.com',
        u'Times Square New Year',
        u'Mika Brzezinksi',
        u'Ayatollah Ali Khamenei',
        u'Nikki Haley',
        u'Obama Library',
        u'internet-based works',
        u'Quadriga',
        u'Washington Post',
        u'Angela Merkel',
        u'Manhattan',
        ...
        u'Germany',
        u'donor',
        u'television terminology',
        u'Bloom',
        u'The Washington Post',
        u'Jack Shafer',
        u'Bei den Veranstaltungen',
        u'singles',
        u'uprising',
        u'reporting',
        u'AP',
    ...
    second_split_row_keys = [
        u'Stockton University',
        u'Walter Montelione',
        u'law enforcement',
        u'shooting',
        u'international incidents',
        u'NYE',
        u'Linda Kologi',
        u'criminal law',
        u'Long Branch Police Department',
        u'Kaitlyn Schallhorn',
        u'Brittany Kologi',
        u'suspect',
        u'teenager',
        u'Monmouth County',
        ...
        u'incident',
        u'WCBS-TV',
        u'Christopher j. Gramiccioni',
        u"Diane D'Amico",
        u'New Jersey',
        u'shooter',
        u'maritime incidents',
        u'Monmouth County Prosecutor',
        u'Steven Kologi',
        u'Bryan Llenas',
        u'Mary Schultz',
        u'NJ.com',
...
def test_or_filtering(tempdir):
    ...
    ref_df = pd.concat([up_df, down_df]).sort_values(cols).reset_index(drop=True)
    # Reading row groups using OR operation in `filters`.
    or_filter = [up_filter, down_filter]
    or_df = pf.to_pandas(filters=or_filter).sort_values(cols).reset_index(drop=True)
...
def test_row_filter_nulls(tempdir):
    ...
    df = pd.DataFrame(
        {"col": [0, 1, np.nan, np.nan]}, index=pd.Index(np.arange(4), name="index")
    )
